---
{
  "id": "bk.dictionary.seed",
  "level": 2,
  "title": "The first words at level two",
  "subject": {
    "kind": "word",
    "target": "accident"
  },
  "records": {
    "dict.2.accident": {"concepts": ["accident"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:accident"}, "defines": {"kind": "word", "target": "accident"}},
    "dict.2.account": {"concepts": ["reporting"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:account"}, "defines": {"kind": "word", "target": "account"}},
    "dict.2.add": {"concepts": ["addition"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:add"}, "defines": {"kind": "word", "target": "add"}},
    "dict.2.already": {"concepts": ["sequence"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:already"}, "defines": {"kind": "word", "target": "already"}},
    "dict.2.amount": {"concepts": ["quantity"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:amount"}, "defines": {"kind": "word", "target": "amount"}},
    "dict.2.balance": {"concepts": ["balance"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:balance"}, "defines": {"kind": "word", "target": "balance"}},
    "dict.2.bicycle": {"concepts": ["household_object"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:bicycle"}, "defines": {"kind": "word", "target": "bicycle"}},
    "dict.2.bunch": {"concepts": ["grouping"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:bunch"}, "defines": {"kind": "word", "target": "bunch"}},
    "dict.2.chain": {"concepts": ["causal_chain"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:chain"}, "defines": {"kind": "word", "target": "chain"}},
    "dict.2.claim": {"concepts": ["claim_under_test"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:claim"}, "defines": {"kind": "word", "target": "claim"}},
    "dict.2.column": {"concepts": ["place_value"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:column"}, "defines": {"kind": "word", "target": "column"}},
    "dict.2.crash": {"concepts": ["accident"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:crash"}, "defines": {"kind": "word", "target": "crash"}},
    "dict.2.digit": {"concepts": ["place_value"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:digit"}, "defines": {"kind": "word", "target": "digit"}},
    "dict.2.eleven": {"concepts": ["natural_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:eleven"}, "defines": {"kind": "word", "target": "eleven"}},
    "dict.2.explanation": {"concepts": ["explanation"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:explanation"}, "defines": {"kind": "word", "target": "explanation"}},
    "dict.2.fail": {"concepts": ["failing"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:fail"}, "defines": {"kind": "word", "target": "fail"}},
    "dict.2.fifty": {"concepts": ["natural_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:fifty"}, "defines": {"kind": "word", "target": "fifty"}},
    "dict.2.forever": {"concepts": ["duration"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:forever"}, "defines": {"kind": "word", "target": "forever"}},
    "dict.2.forty": {"concepts": ["natural_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:forty"}, "defines": {"kind": "word", "target": "forty"}},
    "dict.2.group": {"concepts": ["grouping"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:group"}, "defines": {"kind": "word", "target": "group"}},
    "dict.2.hundred": {"concepts": ["number_word_order"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:hundred"}, "defines": {"kind": "word", "target": "hundred"}},
    "dict.2.knock": {"concepts": ["causal_chain"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:knock"}, "defines": {"kind": "word", "target": "knock"}},
    "dict.2.lead": {"concepts": ["causal_chain"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:lead"}, "defines": {"kind": "word", "target": "lead"}},
    "dict.2.ledger": {"concepts": ["ledger"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:ledger"}, "defines": {"kind": "word", "target": "ledger"}},
    "dict.2.minus": {"concepts": ["subtraction"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:minus"}, "defines": {"kind": "word", "target": "minus"}},
    "dict.2.mishap": {"concepts": ["accident"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:mishap"}, "defines": {"kind": "word", "target": "mishap"}},
    "dict.2.pile": {"concepts": ["grouping"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:pile"}, "defines": {"kind": "word", "target": "pile"}},
    "dict.2.quantity": {"concepts": ["quantity"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:quantity"}, "defines": {"kind": "word", "target": "quantity"}},
    "dict.2.rearrange": {"concepts": ["conservation_of_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:rearrange"}, "defines": {"kind": "word", "target": "rearrange"}},
    "dict.2.reconcile": {"concepts": ["reconciliation"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:reconcile"}, "defines": {"kind": "word", "target": "reconcile"}},
    "dict.2.remove": {"concepts": ["subtraction"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:remove"}, "defines": {"kind": "word", "target": "remove"}},
    "dict.2.set": {"concepts": ["activity"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:set"}, "defines": {"kind": "word", "target": "set"}},
    "dict.2.stock": {"concepts": ["stock_and_flow"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:stock"}, "defines": {"kind": "word", "target": "stock"}},
    "dict.2.subtract": {"concepts": ["subtraction"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:subtract"}, "defines": {"kind": "word", "target": "subtract"}},
    "dict.2.system": {"concepts": ["component_and_system"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:system"}, "defines": {"kind": "word", "target": "system"}},
    "dict.2.thirty": {"concepts": ["natural_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:thirty"}, "defines": {"kind": "word", "target": "thirty"}},
    "dict.2.twelve": {"concepts": ["natural_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:twelve"}, "defines": {"kind": "word", "target": "twelve"}},
    "dict.2.twenty": {"concepts": ["natural_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:twenty"}, "defines": {"kind": "word", "target": "twenty"}},
    "dict.2.unchanged": {"concepts": ["conservation_of_number"], "claim_class": "formal", "provenance": {"source_claim": "lexicon:unchanged"}, "defines": {"kind": "word", "target": "unchanged"}}
  }
}
---

[dict.2.accident]
An accident is a bad thing that happens when nobody wanted it to.

[dict.2.account]
What someone says happened, told in order.

[dict.2.add]
To add is to put one amount with another to make a bigger amount.

[dict.2.already]
Already means before now.

[dict.2.amount]
An amount is how much there is of a thing.

[dict.2.balance]
When two sides hold the same amount, so one is not more than the other.

[dict.2.bicycle]
A bicycle is a thing with two wheels that you use to move with your feet.

[dict.2.bunch]
A bunch is a group of things that are together.

[dict.2.chain]
A chain is many things joined one after another.

[dict.2.claim]
Something said to be true, before anyone has checked it.

[dict.2.column]
A column is a line of things going up and down.

[dict.2.crash]
A crash is when two things hit each other hard.

[dict.2.digit]
A digit is one of the marks you write to show a number.

[dict.2.eleven]
Eleven is one more than ten.

[dict.2.explanation]
What makes a thing clear, by saying why it is so.

[dict.2.fail]
To fail is to try a thing and not do it.

[dict.2.fifty]
Fifty is five tens.

[dict.2.forever]
Forever means for all time, with no end.

[dict.2.forty]
Forty is four tens.

[dict.2.group]
A group is many things or people together.

[dict.2.hundred]
A hundred is ten tens.

[dict.2.knock]
To knock is to hit a thing to make a sound.

[dict.2.lead]
To lead is to go first so others come after.

[dict.2.ledger]
The book a count is kept in, so it is not lost.

[dict.2.minus]
Minus means one amount taken away from another.

[dict.2.mishap]
A mishap is a small accident.

[dict.2.pile]
A pile is many things put one on top of another.

[dict.2.quantity]
A quantity is how many or how much of a thing there is.

[dict.2.rearrange]
To rearrange is to put things in a new order.

[dict.2.reconcile]
To check two counts against each other until they agree.

[dict.2.remove]
To remove is to take a thing away.

[dict.2.set]
A set is a group of things you think of together.

[dict.2.stock]
How much of a thing is held at one time.

[dict.2.subtract]
To subtract is to take one amount away from another.

[dict.2.system]
A system is many parts that work together as one thing.

[dict.2.thirty]
Thirty is three tens.

[dict.2.twelve]
Twelve is two more than ten.

[dict.2.twenty]
Twenty is two tens.

[dict.2.unchanged]
Unchanged means still the same as before.
